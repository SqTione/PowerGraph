...
<main class="container" id="meter">
  ...
  <div class="section__title">
    <h1 id="meter-name" data-id="<?php echo $meter->id; ?>"
        contenteditable="false"><?php echo CHtml::encode($meter->name) ?></h1>
  ...
  <div class="graph">
    ...
    <div class="graph__legend legend">
      <div class="legend__item">
        <div class="legend__image"><img src="<?php Yii::app()->request->baseUrl; ?>/images/icons/phase_a.svg" alt="">
        </div>
        <p class="legend__label">Фаза A</p>
      </div>
      <div class="legend__item">
        <div class="legend__image"><img src="<?php Yii::app()->request->baseUrl; ?>/images/icons/phase_b.svg" alt="">
        </div>
        <p class="legend__label">Фаза B</p>
      </div>
      <div class="legend__item">
        <div class="legend__image"><img src="<?php Yii::app()->request->baseUrl; ?>/images/icons/phase_c.svg" alt="">
        </div>
...
<script type="text/javascript">
  ...
  $(document).ready(function () {
    ...
    $('input[name="graph__period"]').on('change', handlePeriodChange);
    ...
    if ($startDateInput.length && $endDateInput.length) {
      $startDateInput.on('change', handleCustomDateChange);
      $endDateInput.on('change', handleCustomDateChange);
    }
  });

  // Создает и настраивает новый график
  function createChart(chartData) {
    const ctx = $('#voltage-chart')[0].getContext('2d');
